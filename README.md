...
- PHP 7.x hoặc PHP 8.x (đã cài sẵn trong XAMPP)
...
- CSS3
- JavaScript
- MySQL (sử dụng MySQL trong XAMPP)
...
2. Copy toàn bộ thư mục `DOAN2_UPDATE3/admin` vào `xampp/htdocs/`.
3. Mở `http://localhost/phpmyadmin`.
4. Tạo database mới, ví dụ `shoes_store`.
5. Import file `doancs2_shoes (3).sql` vào database mới tạo.
6. Mở file `admin/admin/config/dbconfig.php` và cập nhật kết nối:
   - `host` = `localhost`
   - `username` = `root`
   - `password` = ``
   - `database` = `shoes_store`
7. Mở trình duyệt:
   - `http://localhost/admin/USERS/index.php` để truy cập giao diện khách hàng
   - `http://localhost/admin/admin/index.php` để truy cập trang quản trị
